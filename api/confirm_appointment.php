...
$stmt = $conn->prepare("
    ...
    WHERE action_token = ? AND action_type = 'confirm'
    LIMIT 1
");
...
while ($appt = $resAppts->fetch_assoc()) {
    ...
    $updAppt = $conn->prepare("
        UPDATE appointments
        SET status = 'Confirmado', payload_json = ?
        WHERE external_id = ?
    ");
    ...
    $log = $conn->prepare("
        ...
        VALUES (?, ?, ?, ?, 'CONFIRMACAO_CONSULTA', ?)
    ");
    $descricaoLog = 'Consulta confirmada pelo paciente';
    ...
    if (!empty($appt['google_event_id'])) {
        $titulo = 'Consulta Confirmada - ' . ($appt['paciente_nome'] ?: 'Paciente');
...
<body style="margin:0;padding:0;background:#f6f1f3;font-family:Arial,Helvetica,sans-serif;">
    <div style="max-width:640px;margin:40px auto;background:#fff;border-radius:20px;overflow:hidden;box-shadow:0 12px 40px rgba(0,0,0,.08);">
        ...
        <div style="padding:32px;">
            <h2 style="color:#7b5238;margin-top:0;">Consulta(s) confirmada(s) com sucesso</h2>
            <p style="color:#4b5563;font-size:15px;line-height:1.7;">
                Total de sessões confirmadas: <strong>{$updatedCount}</strong>.
            </p>
        </div>
    </div>
</body>
